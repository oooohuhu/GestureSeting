...
import com.wangy.wiperswitch.utils.StringUtils;
...
public  class Drawl extends View {
    ...
    public Drawl(Context context, List<Point> list, String passWord, float width, GestureCallBack callBack) {
        super(context);
        paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
        bitmap = Bitmap.createBitmap((int)width, 800, Bitmap.Config.ARGB_8888); // 设置位图的宽高
        canvas = new Canvas();
        canvas.setBitmap(bitmap);

        paint.setStyle(Paint.Style.STROKE);// 设置非填充
        paint.setStrokeWidth(10);// 笔宽5像素
        paint.setColor(Color.parseColor("#ffffff"));// 设置颜色
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (pointAt == null || currentPoint.equals(pointAt)
                    ...
                    canvas.drawLine(currentPoint.getCenterX(),
                            currentPoint.getCenterY()+50, event.getX(), event.getY()+50,
                            paint);// 画线
                } else {
                    // 如果当前点击的点与当前移动到的点的位置不同
                    // 那么以前前点的中心为起点，以手移动到的点的位置画线
                    canvas.drawLine(currentPoint.getCenterX(),
                            currentPoint.getCenterY()+50, pointAt.getCenterX(),
                            pointAt.getCenterY()+50, paint);// 画线
    ...
    private void clearScreenAndDrawList() {
        ...
        for (Pair<Point, Point> pair : lineList) {
            canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY()+50,
                    pair.second.getCenterX(), pair.second.getCenterY()+50, paint);// 画线
        }
    }
